...
class Messaging:
    ...
    def send(self, message):
        try:
            ...
            print(f"({self.client.getpeername()[0]}) [>>]: Packet ID: {message.id} Length: {len(message.buffer)}")
            header = Messaging.writeHeader(message.id, len(message.buffer))
            self.client.send(header + message.buffer)
        except Exception as e:
            print(e) 

    @staticmethod
    def writeHeader(type, length):
        buffer = b''
        buffer += int.to_bytes(type, 2, "big")
        buffer += int.to_bytes(length, 3, "big")
        buffer += int.to_bytes(0, 2, "big")
        return buffer
